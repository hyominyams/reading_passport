...
[Story    [Hidden     [Talk with
 ...
 도장1      도장2        도장3
      ↓ (3개 완료 후)
[My Story]
  → Step 1: 활동 선택 + 가이드 질문 (내용/인물/세계)
  → Step 2: 이야기 쓰기 (학생 자유 작성 + 시스템 피드백)
...
stories.current_step 값 기반 라우팅:
  1 → /book/[id]/mystory            (활동 선택 + 가이드 질문)
  2 → /book/[id]/mystory/write      (이야기 쓰기)
...
| 이야기 쓰기 피드백 | 소형 저렴 모델 | Step 2 저장 시 1회 |
| 소형모델 검증 | 소형 저렴 모델 | Step 2→3 전환 시 1회 |
| 초안 + 조언 동시 생성 | GPT-5-mini | Step 2→3 전환 시 1회 |
...
5단계 ── My Story Step 1~2
         유형 선택 + 가이드 질문 + 자유 작성 + 피드백
