...
export async function POST(request: NextRequest) {
  try {
    ...
    // NOTE: Ideally, the full book text (book_full_text) should be passed from the client
    // when available to improve draft quality. Currently using story_summary + characters
    // as an approximation.
...
3. 3~5개 구역으로 나눠 작성하세요. 구역 구분은 반드시 [PAGE_BREAK] 태그를 사용하세요.
4. 각 구역은 2~4문장으로만 구성하세요.
5. 학생이 제공한 재료(등장인물, 배경, 사건, 결말)를 최대한 반영하세요.
