    ...
    <header className="bg-white/80 backdrop-blur-md border-b border-border/60 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-14">
          {/* Logo */}
          <Link href={isAuthenticated ? '/map' : '/'} className="flex min-h-11 items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-foreground flex items-center justify-center">
              <svg className="w-3.5 h-3.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5a17.92 17.92 0 01-8.716-2.247m0 0A9 9 0 013 12c0-1.605.42-3.113 1.157-4.418" />
              </svg>
            </div>
            <span className="text-base font-heading font-semibold text-foreground tracking-tight">World Stories</span>
          </Link>
            ...
            <nav className="hidden md:flex items-center gap-1">
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={`inline-flex min-h-11 min-w-11 items-center justify-center gap-1.5 rounded-xl px-3 text-sm whitespace-nowrap transition-colors ${
                      isActive
              ...
              <div className="flex items-center gap-2 ml-2 pl-3 border-l border-border/60">
                ...
                <button
                  ...
                  title="로그아웃"
                  aria-label="로그아웃"
                  className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl px-2 text-xs text-muted hover:bg-foreground/[0.04] hover:text-foreground transition-colors disabled:cursor-not-allowed disabled:text-muted/50 whitespace-nowrap"
                >
                  {signingOut ? (
